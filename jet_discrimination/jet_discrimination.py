...
from input_pipeline import get_mixed_inputs
import convnet
...
with tf.Graph().as_default():
    with tf.name_scope('input'):
        ...
        images, labels = get_mixed_inputs(is_training)
    ...
    try:
        ...
        while not coord.should_stop():
            ...
            # training
            summary, _ = sess.run([merged, train_op], feed_dict={is_training: True,
                                                                 keep_prob: 0.5})
        ...
        print('Done training for %d epochs, %d steps.' % (NUM_EPOCH, step))
    finally:
        train_writer.close()
        test_writer.close()
